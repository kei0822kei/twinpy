...
class TestTwinBoundaryStructure(unittest.TestCase):
        ...
        for twinmode in self.twinmodes:
            for wyckoff in self.wyckoffs:
                tb = TwinBoundaryStructure(
                        ...
                        symbol=self.symbol,
                        wyckoff=wyckoff)
                tb.set_parent(twinmode=twinmode)
                tb.run(
                       dim=self.dim,
                       xshift=self.xshift,
                       yshift=self.yshift,
                       shear_tb_ratio=self.shear_tb_ratio,
                       make_tb_flat=True,
                       )
                # a = tb.get_phonopy_structure(structure_type='base')
                # a = tb.get_phonopy_structure(structure_type='conventional')
                # a = tb.get_phonopy_structure(structure_type='primitive')
        # try:
...
if __name__ == '__main__':
    suite = unittest.TestLoader().loadTestsFromTestCase(TestTwinBoundaryStructure)
